...
export async function getSkillHeatmap(req: Request, res: Response) {
  try {
    const cypher = `
      MATCH (s:Student)-[knows:KNOWS]->(sk:Skill)
    ...
    `;
    const records = await runRead<{ name: string; category: string; count: { low: number; high: number } | number }>(cypher);
...
export async function getIndustryGap(req: Request, res: Response) {
  try {
    const cypher = `
      ...
      OPTIONAL MATCH (s:Student)-[knows:KNOWS]->(sk)
      WHERE coalesce(knows.confidence, 0) >= 0.5 AND coalesce(knows.dormant, false) = false
